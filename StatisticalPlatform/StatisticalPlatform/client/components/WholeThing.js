...
class WholeThing extends Component {
  ...
  componentDidMount() {
    ...
    let table = null;
		...
		// Data table
		container = ReactDOM.findDOMNode(this.refs.data_ref);
		...
		// Univariate table
		container = ReactDOM.findDOMNode(this.refs.uni_ref);
		table = new Handsontable(container, {
			colHeaders: true,
			minSpareRows: 0,
			contextMenu: false,
		...
		// Bivariate table
		container = ReactDOM.findDOMNode(this.refs.bi_ref);
		table = new Handsontable(container, {
			colHeaders: true,
			minSpareRows: 0,
			contextMenu: false,
		...
		// Tests table
		container = ReactDOM.findDOMNode(this.refs.test_ref);
		table = new Handsontable(container, {
			colHeaders: true,
			minSpareRows: 0,
			contextMenu: false,
		...
		// ANOVA table
		container = ReactDOM.findDOMNode(this.refs.anova_ref);
		table = new Handsontable(container, {
			colHeaders: true,
			minSpareRows: 0,
			contextMenu: false,
		...
		// Classify table
		container = ReactDOM.findDOMNode(this.refs.classify_ref);
		table = new Handsontable(container, {
			colHeaders: true,
			minSpareRows: 0,
			contextMenu: false,
  ...
  componentDidUpdate(prevProps, prevState)  {
    ...
    if(this.state.cluster) {
    	...
    	const bundle = {clusters: this.state.clusters, eps: this.state.eps, table: this.props.data_table, vars: {x: this.state.var_x, y: this.state.var_y}};
	    ...
	    if(this.state.classify) {
	    	...
	    	const bundle = {classify_var: this.state.classify_var, table: this.props.data_table, file: this.state.classify_file, ratio: this.state.classify_ratio, classify_type: this.state.classify_type};
  ...
  render() {
    ...
    session.getObject( two.bind(this));
    }
    ...
    const three = (fieldsession) => {

      fieldsession.getObject(four.bind(this));
    }
